...
//   // Fix: Initialize useRef with null as it was called without arguments, which causes a TypeScript error.
...
//   const currentPhotoX = useRef(0);
//   const currentTextX = useRef(0);
//   const currentTextOpacity = useRef(0);
//   const currentTextScale = useRef(0.95);
...
//     const easedProgress = Math.pow(progress, 1.6); // gentle at start, strong near end

//     // Photo slight depth and horizontal drift
//     const targetPhotoX = -17 * easedProgress;
//     currentPhotoX.current += (targetPhotoX - currentPhotoX.current) * LERP_FACTOR;

//     const photoZ = 300 - easedProgress * 300; // subtle forward push
//     const rotationPhoto = easedProgress * -6;

//     // Text cinematic zoom-in
//     const targetTextX = 17 * easedProgress;
//     currentTextX.current += (targetTextX - currentTextX.current) * LERP_FACTOR;

//     // Text depth and scale (starts far behind, rushes forward)
//     const maxDepthText = 1200;
//     const textZ = -maxDepthText + easedProgress * maxDepthText;
//     const textScale = 0.5 + 0.5 * easedProgress; // smoother size ramp
//     const rotationText = easedProgress * 8;

//     // Opacity smooth fade-in
//     const textOpacity = Math.min(easedProgress * 1.5, 1);

//     // Apply transforms
//     if (photoWrapperRef.current && textWrapperRef.current) {
//       photoWrapperRef.current.style.transform = `
//       translate3d(calc(-50% + ${currentPhotoX.current}vw), -50%, ${photoZ}px)
//       rotateY(${rotationPhoto}deg)
//     `;

//       textWrapperRef.current.style.transform = `
//       translate3d(calc(-50% + ${currentTextX.current}vw), -50%, ${textZ}px)
//       scale(${textScale})
