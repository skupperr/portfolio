...
type DemoComputerProps = ComponentPropsWithoutRef<'group'> & {
  texture?: string;
};

const DemoComputer = ({ texture, ...rest }: DemoComputerProps) => {
  const group = useRef<THREE.Group>(null);

  const { nodes, materials, animations } = useGLTF('/model/computer.glb');
  const { actions } = useAnimations(animations, group);

  // Top-level hook usage (valid)
    const originalTxt = useVideoTexture(texture ?? "/prove.mp4") as THREE.VideoTexture | null;
  
    // Create a clone so we don't modify the value returned by the hook
    const txt = useMemo(() => {
      if (!originalTxt) return null;
      const t = originalTxt.clone();
      t.flipY = false;
      t.needsUpdate = true;
      return t;
    }, [originalTxt]);
  
    // Dispose the cloned texture when this component unmounts or the clone changes
    useEffect(() => {
      return () => {
        if (txt) txt.dispose();
      };
